...
CCBreakbeatTest : UnitTest {
  ...
  test_new_storesName {
    var b = CCBreakbeat(\test, (duration: 4.0));
    this.assertEquals(b.name, \test, "Should store name");
  }

  test_new_storesBuffer {
    var buf = (duration: 4.0);
    var b = CCBreakbeat(\test, buf);
    this.assertEquals(b.buffer, buf, "Should store buffer");
  }

  test_new_storesNumSlices {
    var b = CCBreakbeat(\test, (duration: 4.0), 16);
  ...
  test_sliceStart_firstSlice {
    var b = CCBreakbeat(\test, (duration: 4.0), 8);
  ...
  test_pattern_returnsPdef {
    var b = CCBreakbeat(\test, (duration: 4.0), 4).bars(1);
    var pat = b.pattern([0, 1, 2, 3]);
    this.assert(pat.isKindOf(Pdef), "pattern should return a Pdef");
  }

  test_pattern_hasInstrumentKey {
    var b = CCBreakbeat(\test, (duration: 4.0), 4).bars(1);
    var pat = b.pattern([0, 1, 2, 3]);
    var stream = pat.source.asStream;
  ...
  test_pattern_hasBufferKey {
    var buf = "mockBuffer";
    var b = CCBreakbeat(\test, buf, 4).bars(1);
    var pat = b.pattern([0, 1, 2, 3]);
    var stream = pat.source.asStream;
    var event = stream.next(());
  ...
  test_pattern_setsQuantOnPdef_fullOrder {
    var b = CCBreakbeat(\test, (duration: 4.0), 8).bars(1);
    b.pattern((0..7));
    this.assertFloatEquals(Pdef(\test).quant, 4.0, "8 slices * 0.5 dur = 4 beats");
  }

  test_pattern_setsQuantOnPdef_partialOrder {
    var b = CCBreakbeat(\test, (duration: 4.0), 8).bars(1);
    b.pattern([0, 1, 2, 3]);
    this.assertFloatEquals(Pdef(\test).quant, 2.0, "4 slices * 0.5 dur = 2 beats");
  }

  test_pattern_setsQuantOnPdef_twoBars {
    var b = CCBreakbeat(\test, (duration: 4.0), 16).bars(2);
    b.pattern((0..15));
    this.assertFloatEquals(Pdef(\test).quant, 8.0, "16 slices * 0.5 dur = 8 beats");
  }
