...
export const synthdefs: Record<string, SynthDef> = {
  kick: {
    ...
    code: `SynthDef(\\kick, { |out=0, freq=60, amp=0.5, decay=0.3|
  var sig, env;
  ...
  snare: {
    ...
    params: ["freq", "amp", "decay"],
    code: `SynthDef(\\snare, { |out=0, freq=180, amp=0.5, decay=0.2|
  var sig, env, noise;
  env = EnvGen.kr(Env.perc(0.001, decay), doneAction: 2);
  ...
  hihat: {
    ...
    params: ["amp", "decay"],
    code: `SynthDef(\\hihat, { |out=0, amp=0.3, decay=0.05|
  var sig, env;
  env = EnvGen.kr(Env.perc(0.001, decay), doneAction: 2);
  ...
  clap: {
    ...
    params: ["amp", "decay"],
    code: `SynthDef(\\clap, { |out=0, amp=0.5, decay=0.15|
  var sig, env;
  env = EnvGen.kr(Env.perc(0.001, decay), doneAction: 2);
  ...
  bass: {
    ...
    params: ["freq", "amp", "decay"],
    code: `SynthDef(\\bass, { |out=0, freq=55, amp=0.5, decay=0.5, gate=1|
  var sig, env;
  ...
  acid: {
    ...
    code: `SynthDef(\\acid, { |out=0, freq=55, amp=0.5, cutoff=1000, res=0.3, decay=0.3, gate=1|
  var sig, env, fenv;
  ...
  pad: {
    ...
    code: `SynthDef(\\pad, { |out=0, freq=220, amp=0.3, attack=0.5, release=1, gate=1|
  var sig, env;
